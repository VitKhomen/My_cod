...
# Замена на верные буквы


def replace_letter(word, guess, word_completion):
    word_compl = list(word_completion)
    ind_leter = [s for s in range(len(word)) if word[s] == guess]
    for ind in ind_leter:
        word_compl[ind] = guess
    return ''.join(word_compl)
...
def play(word):
    ...
    print()
    # easy_mod(word, word_completion)
    word_completion = easy_mod(word, word_completion)
    print(word_completion)
    print()
    while tries > 0:
        ...
        if guess.isalpha() and len(guess) == 1:
            if guess in guessed_letters:
                print('Эта буква уже была', guess)
            elif guess not in word:
                print('Буквы', guess, 'в слове нет.')
                tries -= 1
                guessed_letters.append(guess)
            else:
                print('Отличная работа, буква', guess, 'есть в слове !')
                guessed_letters.append(guess)
                word_completion = replace_letter(word, guess, word_completion)
    ...
    if guessed:
        print('Поздравляем, вы угадали слово! ' + word + ' Вы победили.')
    else:
        print('Вы не угадали слово. Загаданным словом было '
              + word + '. Может быть в следующий раз!')


# Функция упрощения игры
def easy_mod(word, word_completion):
    word_comp_list = list(word_completion)
    word_list = list(word)
    eas_mod = input(
        'Если хотите упростить игру я могу открыть первую и последнюю букву.(д = да, н = нет): ')
    if eas_mod == 'д':
        word_comp_list[0] = word_list[0]
        word_comp_list[-1] = word_list[-1]
    else:
        print('Как пожелаете.')
    return ''.join(word_comp_list)
